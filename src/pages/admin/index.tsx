        ...
        <div className="flex items-center flex-col min-h-screen pb-7 px-2">
                    ...
                    nameInput && (
                        <div className='flex items-center justify-start flex-col mb-7 p-1 border-gray-100/25 border rounded-md'>
                            ...
                            <article className='w-11/12 max-w-lg flex flex-col items-center justify-between rounded px-1 py-3' style={{ marginBottom: 8, marginTop: 8, backgroundColor: backgroundColorInput }}>
                                <p className='font-medium' style={{ color: textColorInput }}>{nameInput ? nameInput : 'Texto de exemplo'}</p>
                            </article>
                        </div>
                    )
                }
...

            {
                links && links.map((link) => (
                    <article className='flex items-center justify-between w-11/12 max-w-xl rounded py-3 px-2 mb-2 select-none' style={{ marginBottom: 8, marginTop: 8, backgroundColor: link.bg, color: link.color }} key={link.id}>
                        <p className='font-medium'>{link.name}</p>
                        <div>
                            <button className='border border-dashed p-2 rounded bg-neutral-900'
